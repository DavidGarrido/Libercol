<template>
  <div>
    <div class="fixed pointer-events-none  left-0 right-0 top-0 flex items-center justify-center notif-parent">
      <div v-if="$page.props.flash.success && show" class="flex items-center justify-between bg-green-500 rounded max-w-3xl my-8 px-10 shadow-xl">
    ...
    <div class="fixed left-0 right-0 top-0 flex items-center justify-center notif-parent">
      <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="flex items-center justify-between bg-red-400 rounded max-w-3xl my-8">
        <div class="flex items-center">
...
export default {
  ...
  methods: {
    hide(){
      document.querySelectorAll('.notif-parent').forEach((element) => {
        setTimeout(function(){
